...
CGO_ENABLED=0 GOOS=linux GOARCH="$arch" go build -mod=readonly -trimpath -buildvcs=false -ldflags="-s -w" -o "$helper.tmp" ./cmd/storage-helper
mv "$helper.tmp" "$helper"
;;
  application)
sha256sum "$helper" | cut -d ' ' -f 1 > "$helper.sha256.tmp"
...
cd "$root/frontend"
npm ci
npm run build -- --outDir "$build_root/frontend/dist" --emptyOutDir
cd "$root/backend"
...
CGO_ENABLED=0 GOOS=linux GOARCH="$arch" go build -mod=readonly -buildvcs=false -tags replicaro_embedded_ui -trimpath -overlay "$product_overlay" -o "$build_root/replicaro-$target" .
;;
