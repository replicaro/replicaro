...
// Each Unix build includes exactly one target directory selected by the
// build-specific targetAssetPath constant.
var embeddedUnixBinaries embed.FS

func extractEmbeddedBinary(id, name string) (string, error) {
	data, err := embeddedUnixBinaries.ReadFile(filepath.ToSlash(filepath.Join("assets", targetAssetPath, id)))
	if err != nil {
		return "", err
	}
	expected, err := expectedEmbeddedHash(id, targetAssetPath)
	if err != nil {
		return "", err
	}
	...
	if err := appdata.AtomicReplace(temporaryPath, path); err != nil {
		return "", fmt.Errorf("install embedded %s for %s/%s: %w", id, runtime.GOOS, runtime.GOARCH, err)
	}
	if err := os.Chmod(path, 0o700); err != nil {
		return "", err
	}
